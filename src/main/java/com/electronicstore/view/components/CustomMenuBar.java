...
public class CustomMenuBar extends MenuBar {
    ...
    private void handleLogout() {
        if (AlertDialog.showConfirmation("Logout", "Are you sure you want to logout?")) {
            app.logout(); // ✅ deterministic, test-friendly
        }
    }
    ...
    // Event handlers (unchanged behavior)
    public void setOnNewBill(Runnable action) {
        MenuItem mi = findMenuItem("New Bill");
        if (mi != null) mi.setOnAction(e -> action.run());
    }

    public void setOnViewBills(Runnable action) {
        MenuItem mi = findMenuItem("View Today's Bills");
        if (mi != null) mi.setOnAction(e -> action.run());
    }

    public void setOnInventory(Runnable action) {
        MenuItem mi = findMenuItem("Inventory Management");
        if (mi != null) mi.setOnAction(e -> action.run());
    }
