...
class SupplierFileIntegrationTest {
    ...
    void saveAndReadSupplier_successfully() {
        ...
        // WHEN
        assertDoesNotThrow(() ->
                FileHandler.saveListToFile(suppliers, testFile)
        );

        List<Supplier> loadedSuppliers = assertDoesNotThrow(() ->
                FileHandler.readListFromFile(testFile)
        );

        // THEN
        assertNotNull(loadedSuppliers);
